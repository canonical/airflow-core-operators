...
def test_pebble_connection_failure_scenario(context, state, container, triggerer_relation):
    """When the container cannot connect to Pebble."""
    container = dataclasses.replace(container, can_connect=False)
    state_in = dataclasses.replace(state, relations=[triggerer_relation], containers=[container])
...
def test_waiting_when_cannot_write_airflow_config(context, state, container, triggerer_relation):
...
def test_replan_failure_scenario_test(context, state, container, triggerer_relation):
...
def test_replan_failure_scenario(context, state, container, triggerer_relation):
...
def test_active_status_flow_scenario(context, state, container, triggerer_relation):
    ...
    out_container = state_out.get_container(constants.CONTAINER_NAME)
    layer = out_container.layers["triggerer-base"]
    ...
    assert layer.services[constants.SERVICE_NAME].user == constants.WORKLOAD_USER
    assert layer.services[constants.SERVICE_NAME].group == constants.WORKLOAD_GROUP
