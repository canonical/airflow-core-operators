...
def test_pebble_connection_failure_scenario(context, state, container, dag_processor_relation):
    """When the container cannot connect to Pebble."""
    container = dataclasses.replace(container, can_connect=False)
    state_in = dataclasses.replace(
        state, relations=[dag_processor_relation], containers=[container]
    )
...
def test_missing_relation_status_scenario(context, state, container):
    ...
    assert "dag-processor-base" not in out_container.layers
...
def test_waiting_when_cannot_write_airflow_config(
...
def test_replan_failure_scenario(context, state, container, dag_processor_relation):
...
def test_active_status_flow_scenario(context, state, container, dag_processor_relation):
    ...
    out_container = state_out.get_container(constants.CONTAINER_NAME)
    layer = out_container.layers["dag-processor-base"]
